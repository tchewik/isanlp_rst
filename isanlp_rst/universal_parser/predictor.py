...
class Predictor:
    def __init__(self,
        ...
        for corpus_name in ast.literal_eval(self.config['data']['corpora']):
            _corpus_name = corpus_name.lower()
            if _corpus_name == 'rst-dt-tr':
                _corpus_name = 'rst-dt'
            elif _corpus_name == 'gum10-tr':
                _corpus_name = 'gum'

            if self.mode == 'local':
                relation_table_path = os.path.join(model_dir, f'relation_table_{_corpus_name}.txt')

            elif self.mode == 'hf':
                relation_table_path = hf_hub_download(repo_id=self.hf_model_name,
                                                      filename=f'relation_table_{_corpus_name}.txt',
                                                      revision=self.hf_model_version)

            relation_table = open(relation_table_path, 'r').read().splitlines()
    ...
    def _load_model(self):
        ...
        model_config = {
            ...
            'classes_numbers': [len(table) for table in self.relation_tables],
    ...
    def parse_rst(self, text: str):
        ...
        for token in tree.text.split(' '):
            ...
            begin += len(token) + 1
        # for token in data['input_sentences'][0]:
        #     tokens.append(Token(text=token, begin=begin, end=begin + len(token)))
        #     begin += len(token) + 1
