...
top_classes = labels  # Could use a relation subset
...
def metric_parseval(parsed_pairs, gold, span=True, labeled=False, nuc=False):
    ...
    for i in parsed_pairs.index:
        if span:
            x, y = charsonly(prepare_string(parsed_pairs.loc[i, 'snippet_x'])
                             ), charsonly(prepare_string(parsed_pairs.loc[i, 'snippet_y']))
    ...
    for i in gold.index:
        if span:
            x, y = charsonly(prepare_string(gold.loc[i, 'snippet_x'])
                             ), charsonly(prepare_string(gold.loc[i, 'snippet_y']))
    ...
    parsed_strings = set(
        [parsed_strings[i] for i in range(len(parsed_strings)) if not i in _remove_from_parsed_strings])
...
def metric_parseval_df(parsed_pairs, gold, span=True, labeled=False, nuc=False):
    ...
    for i in gold.index:
        if span:
            x, y = charsonly(gold.loc[i, 'snippet_x']), charsonly(gold.loc[i, 'snippet_y'])
...
def extr_pairs(tree):
    tt = []
    if tree.left:
        tt += [[tree.left.text, tree.right.text, tree.relation, tree.nuclearity]]
        tt += extr_pairs(tree.left)
        tt += extr_pairs(tree.right)
    return tt
...
def eval_segmentation(trees, _gold_edus, verbose=False):
    ...
    for gold_edu in _gold_edus:
        gold_edus.append(charsonly(gold_edu))
...
def eval_pipeline(trees=None, gold_edus=[], gold_pairs=pd.DataFrame([]), text="", parsed_pairs=pd.DataFrame([]),
    ...
    result['full_true_pred'], result['full_all_pred'], result['full_all_true'] = metric_parseval(parsed_pairs,
                                                                                                 gold_pairs,
