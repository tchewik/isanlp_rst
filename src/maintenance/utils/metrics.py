...
class DiscourseMetric(Metric):
    ...
    def __call__(self, preds, golds):
        for pred in preds:
            max_us_tp = 0
            max_r_tp, max_n_tp = 0, 0
            max_span_pred, max_span_gold = 0, 0

            for gold in golds:
                span_pred = self.get_span_label(pred)
                span_gold = self.get_span_label(gold)
                # print(span_pred, span_gold)
                ...
                if len(us_tp) > max_us_tp:
                    max_us_tp = len(us_tp)
                    max_span_pred = len(span_pred)
                    max_span_gold = len(span_gold)
                    ...
                    max_r_tp = len(list((r_pred & r_gold).elements()))
                    ...
                    max_n_tp = len(list((n_pred & n_gold).elements()))

            self.n += 1
            # self.n_ucm += len(utp) == len(pred) == len(gold)
            # self.n_lcm += len(ltp) == len(pred) == len(gold)
            self.us_tp += max_us_tp
            self.r_tp += max_r_tp
            self.n_tp += max_n_tp
            self.pred += max_span_pred
            self.gold += max_span_gold
...
class DiscourseMetricDoc(Metric):
    ...
    def __call__(self, preds, golds):
        for pred in preds:
            ...
            self.r_tp += max_r_tp
            self.n_tp += max_n_tp
            self.rn_tp += max_rn_tp
            self.seg_tp += max_seg_tp
            self.pred += max_span_pred
            self.gold += max_span_gold
        ...
        if text_tree == 'NONE':
            ...
            for each_split in metric_token_split:
                if each_split:
                    ...
                    # segmentpoints.append(right_end)
                    segmentpoints.append(left_end)
                    # segmentpoints = list(set(segmentpoints))
