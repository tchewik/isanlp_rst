...
class AllenNLPCustomBiMPMClassifier(SimpleAllenNLPClassifier):
    ...
    def predict_proba(self, snippet_x, snippet_y, additional_features, *args, **kwargs):
        _snippet_x = self._prepare_sequence(snippet_x, is_left_snippet=True)
        _snippet_y = self._prepare_sequence(snippet_y, is_left_snippet=True)
        ...
        return self._model.predict(_snippet_x, _snippet_y, additional_features)['probs']

    def predict_proba_batch(self, snippet_x, snippet_y, additional_features, *args, **kwargs):
        predictions = self._model.predict_batch_json([
            {'premise': self._prepare_sequence(snippet_x[i], is_left_snippet=True),
             'hypothesis': self._prepare_sequence(snippet_y[i], is_left_snippet=False),
             'metadata': additional_features[i]} if 0 < len(
                snippet_x[i]) and 0 < len(snippet_y[i]) else
    ...
    def predict(self, snippet_x, snippet_y, additional_features, *args, **kwargs):
        _snippet_x = self._prepare_sequence(snippet_x, is_left_snippet=True)
        _snippet_y = self._prepare_sequence(snippet_y, is_left_snippet=True)
        ...
        return self._model.predict(_snippet_x, _snippet_y, additional_features)['label']

    def predict_batch(self, snippet_x, snippet_y, additional_features, *args, **kwargs):
        predictions = self._model.predict_batch_json([
            {'premise': self._prepare_sequence(snippet_x[i], is_left_snippet=True),
             'hypothesis': self._prepare_sequence(snippet_y[i], is_left_snippet=False),
             'metadata': additional_features[i]} if 0 < len(
